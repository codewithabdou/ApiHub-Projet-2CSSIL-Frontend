...
import {revenuePerMonth} from "@services/api/statistics/getRevenuPerMonth";
import {revenuePerDay} from "@services/api/statistics/getRevenuePerDay";
import {revenuePerHour} from "@services/api/statistics/getRevenuePerHour";
import {totalRevenu} from "@services/api/statistics/getTotalRevenue";
import StatCard from "@app/components/Shared/StatCard";
import { IMAGES } from "@config";
import months from "@helpers/statsHelpers";
import totalApiNumberGeneral from "@services/api/statistics/totalApisNumberGeneral";
import usersStats from "@services/api/statistics/usersStats";
...
const AdminDashboard = () => {
  ...
  const [nbSupps, setNbSupps] = useState(0);
  ...
  const [text,setText]=useState("Revenue total par mois");
   useEffect(()=>{
    const getTotal=async ()=>{
      const result=await totalRevenu();
      setTotal(result)
    }
    const getGenStats=async()=>{
      const result=await totalApiNumberGeneral();
      const res=await usersStats();
      setNbApis((result));
      setNbAdmins(res.admins_number)
      setNbSupps(res.suppliers_number)
      setNbUsers(res.users_number)
      console.log
      (result)
    }
    getTotal()
  ...
  const perMonth = async (year: string) => {
    const result:any = await revenuePerMonth(year);
  ...
  const perDay=async(month:string,year:string)=>{
    const result:any=await revenuePerDay(year,month);
    const daysInMonth = new Date(parseInt(year), parseInt(month), 0).getDate();
  ...
  const perHour=async(month:string,year:string,day:string)=>{
    const result:any=await revenuePerHour(year,month,day);
  ...
  return (
    <div className="flex flex-col justify-center items-center gap-y-7">
      ...
      <div className="w-full  flex items-center justify-around flex-wrap gap-5">
        <StatCard description="Total Utilisateurs" value={nbUsers} icon={IMAGES.STATS.users}></StatCard>
        <StatCard description="Total fournisseurs" value={nbSupps} icon={IMAGES.STATS.users}></StatCard>
        <StatCard description="Total APIs" value={nbApis} icon={IMAGES.STATS.purchase}></StatCard>
        <StatCard description="Total admins" value={nbAdmins} icon={IMAGES.STATS.admin}></StatCard>
