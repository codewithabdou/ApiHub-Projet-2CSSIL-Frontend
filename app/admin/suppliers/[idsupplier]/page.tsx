...
<div>
  <h1 className='text-primary text-xl lg:text-4xl font-bold align-top mb-9'>Information personnels :</h1>
...
<div className="grid w-full max-w-sm items-center gap-1.5">
      ...
      <Input  id="prenom" value={user?.firstname ?? ""} placeholder="Prenom" disabled />
    </div>

    <div className="grid w-full max-w-sm items-center gap-1.5">
      <Label htmlFor="nom">Nom</Label>
      <Input id="nom" value={user?.lastname ?? ""} placeholder="Nom" disabled />
    </div>
...
<Input  id="phone_number" value={user?.phone_number ?? ""} placeholder="phone_number" disabled />
</div>
</div>
...
<div>
<h1 className='text-primary text-xl lg:text-4xl font-bold align-top mb-9'>APIs publiées:</h1>
    ...
    <CarouselContent>
      {  supplierAPI &&  supplierAPI?.length ? (
        supplierAPI.map((api, index) => (
          <CarouselItem key={index} className="    md:w-1/2 lg:w-1/3 flex   justify-center gap-4 w-full md:pl-4 md:basis-1/2 lg:basis-1/3">
          <Card className="w-[250px] h-[300px] flex flex-col justify-between">
            <CardHeader className="h-[80px] flex flex-row gap-4 items-center">
              ...
              <div>
                <CardTitle className="text-center ">{api.name}</CardTitle>
                <CardDescription className="text-center ">{api.category.name}</CardDescription>
              </div>
            </CardHeader>
            <CardContent className="h-[120px] overflow-y-auto">
              <div className="max-w-full text-foreground">{api.description}</div>
            </CardContent>
      ...
      : (
        <div className="text-center w-full">Aucune API publiée par ce fournisseur</div>
      )}
    </CarouselContent>
