...
const DiscussionsSection= async (props:any)=> {
  ...
  let data:Disscussion[]=[];
      ...
      const result:{data:Disscussion[],status:string,message:string} | ErrorType= await getAllDiscussions(props.id);
      if (result.status==="success") {
        console.log(result);
        
        const res=result as {data:Disscussion[],status:string,message:string} ;
        data=res.data;
