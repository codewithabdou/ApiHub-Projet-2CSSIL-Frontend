...
import { createApiFormSchema, createApiRequest } from '@typings/api/createApiTypes'
...
function CreateApiForm() {
    ...
    const [durations,setDurations]=useState([{value:"Par semaine",duration:7},{value:"Par mois",duration:30},{value:"Par an",duration:365},{value:"Illimitée",duration:999999}])
 ...
 function transformDataToApiStructure(data:any)  {
    ...
    data.plans = data.plans.map((plan: { price: string; max_requests: string; duration: string }) => {
      return {
        ...plan,
     ...
     //Submit function that uses the backend API
      ...
      const planFields=Array.from({length:nbOfPlans},(_,index)=>(
        ...
        <div className=" md:w-[80%] md:text-lg text-sm w-full bg-white rounded-[20px] shadow-md  flex flex-col p-5 border-[1px]">
            ...
            <h1>Plan {index+1} :</h1>
            <Button type='button' onClick={deletePlan}>Annuler</Button>
            </div>
        ...
        render={({field})=>(
            <FormItem>
                ...
                <FormControl>
                    <Textarea placeholder="Description du plan" {...field}></Textarea>
                </FormControl>
                <FormMessage/>
            </FormItem>
            )}/>
        ...
        render={({field})=>(
            <FormItem>
                ...
                <FormControl>
                    <Input placeholder="Nombre maximal de requettes" {...field}></Input>
                </FormControl>
                <FormMessage/>
            </FormItem>
            )}/>
                        ...
                        <FormControl>
                            ...
                            <SelectContent>
                                {durations.map((dur,key)=>(
                                    <SelectItem  key={key} value={`${dur.duration}`}>{dur.value}</SelectItem>
                                ))}
                            </SelectContent>
                        </Select>
                    <FormMessage/>
        ...
        className="space-y-8 bg-white px-8 py-12 rounded-lg shadow-md w-full">
                        ...
                        <FormControl>
                            ...
                            <SelectContent>
                                {categories.map((cat,key)=>(
                                    <SelectItem  key={key} value={`${cat.id}`}>{cat.name}</SelectItem>
                                ))}
                            </SelectContent>
                        </Select>
                    <FormMessage/>
            ...
            render={({field})=>(
                <FormItem>
                    ...
                    <FormControl>
                        <Textarea placeholder="Description de l'Api" {...field}></Textarea>
                    </FormControl>
                    <FormMessage/>
                </FormItem>
            )}/>
           ...
           {planFields}
            ...
            <h1 className='font-bold'>Ajouter un plan</h1>
        <Button type='button' onClick={()=>{setNbPlans(nbOfPlans+1)}}>Ajouter un plan</Button>
            </div>
