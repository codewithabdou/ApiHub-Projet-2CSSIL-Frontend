...
//todo : add pagination after it is added to the backend , and modify the getAllCategores function
  ...
  const data = await getAllCategories();
    if (data.status === "success") {
        fetchedCategories = (data as getAllCategoriesSuccessResponse).data;
    } else {
...
<section className="flex flex-col justify-center gap-5 border border-red-800 px-24">

          <MainTitle title="Toutes les categories:" />
          <p>Vous trouvez dans cette section toutes les categories d’APIs disponibles sur  1001 API.</p>
          <div className="grid grid-cols-1  gap-x-4 gap-y-5 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 lg:px-28  min-h-96">
            {fetchedCategories.length > 0 ? (
              ...
              <p className="text-center  col-span-4 row-span-2">Aucun résultat trouvé</p>
            )}
          </div>
