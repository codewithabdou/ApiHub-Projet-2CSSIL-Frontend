...
async function revenuePerMonthSup(year:string): Promise<{
  data: Array<{
    year: number;
    month: number;
    total_revenues: number;
...
try {
  const response = await fetch(
    `${API_INFO.API_BASE_URL}${API_INFO.API_ENDPOINTS.GENERAL.GET_APIS}/subscriptions/mine/revenue/month?year=${year}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
...
export  {revenuePerMonth,revenuePerMonthSup};
